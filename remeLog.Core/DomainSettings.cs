...
{
    /// <summary>
    /// Рантайм-состояние бизнес-настроек (нормативы, праздники, серийные детали), к которым
    /// модели remeLog.Core обращаются напрямую. Заполняется из БД при старте приложения;
    /// сам класс — только хранилище. UI-настройки (профили колонок и прочее) сюда не попадают
    /// и остаются в remeLog.Infrastructure.AppSettings, которая для перечисленных полей
    /// делегирует в этот класс.
    /// </summary>
