...
{
    /// <summary>
    /// Маска фич приложения. В cnc_remelog_feature_permissions.EnabledFeatures пишется СУММА
    /// значений (не номера сдвигов!): только AiMasterCheck — 16, AiMasterCheck+ReasonOverride —
    /// 48, все — 63. Фактические значения проставлены у каждой фичи ниже, чтобы не считать
    /// сдвиги в уме при выдаче прав.
    /// </summary>
    [Flags]
    public enum RemeLogFeature
    {
        ...
        /// </summary>
        AiMasterCheck = 1 << 4,       // = 16
        /// <summary>
        /// Переопределение причин отклонений аналитиком (СГТ 1) через контекстное меню в
        /// PartsInfoWindow. Отметка мастера при этом не правится — решение аналитика ложится
        /// параллельным слоем (Part.SetupReasonOverride и т.д.), в гриде колонка остаётся одна
        /// и показывает эффективную причину. Под флагом только само переопределение; видеть
        /// чужие переопределения (маркер + тултип) могут все роли без флага.
        /// </summary>
        ReasonOverride = 1 << 5,      // = 32
    }
