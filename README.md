...
Поиск (`remeLog/Models/WindchillClient.cs`) реализован через **Windchill REST Services (WRS)** —
официальный OData-эндпоинт Windchill. Один HTTP-запрос отдаёт готовый JSON с результатом.

Окно результатов (`WncObjectsWindow`) даёт три независимых поля поиска и переключатель:
- **Ключевое слово** — комбинированный поиск по наименованию и обозначению сразу (значение
  разбивается на слова, каждое слово ищется подстрокой в любом из полей).
- **Наименование** / **Обозначение** — точный поиск только по одному полю. Для подстрочного
  поиска используется `*` — та же нотация, что и у фильтра "Деталь" в гриде (`*текст*` —
  подстрока где угодно, `*текст`/`текст*` — окончание/начало).
- **Только CAD документы** — ограничивает поиск сущностью `CADDocuments` (3D-модели, детали,
  сборки, чертежи). Без флажка дополнительно опрашиваются и обычные документы Windchill
  (`Documents`) — на практике это служебные бумаги вроде извещений о несоответствии, не чертежи.

Технические детали (адрес OData-сервиса, разбор ответа, лимит на количество результатов) — в
комментариях `WindchillClient.cs`. Кратко: используется сервис `CADDocumentMgmt`
(`/Windchill/servlet/odata/v1/CADDocumentMgmt/CADDocuments`), фильтрация — через `$filter`.
Авторизация — Basic (логин/пароль из `cnc_wnc_cfg`). `LocalType` этой реализацией не
используется — сама сущность `CADDocuments` уже ограничивает выборку CAD-документами, отдельный
фильтр по типу не нужен.

Точного общего числа совпадений сервер не даёт надёжно (`@odata.count` в ответе может расходиться
с фактическим числом строк) — поэтому предупреждение о слишком широком запросе показывается по
факту упора в лимит (`WindchillClient.MaxResults`), без конкретной цифры.

Токен-based (OAuth2) авторизация в WRS существует, но требует отдельной настройки на стороне
Windchill (свой OAuth-клиент, версия WRS не ниже r2.2, возможно внешний IdP) — административная,
не программная задача, и не даёт выигрыша в безопасности, пока сервер работает по `http://`, а не
`https://` (Basic и Bearer токен одинаково идут открытым текстом без TLS). Если понадобится —
начинать с проверки версии WRS и включения HTTPS на сервере, а не с правки кода.

**При следующем обновлении Windchill:** версии OData-сервисов (`v1`, `v2`, `v3`, ...) у разных
сервисов на одном сервере Windchill не совпадают и могут меняться при апгрейде — если поиск
сломается, сначала проверить актуальный путь через
`{Server}/Windchill/servlet/odata/PTC/GetCSRFToken()` (должен отвечать 200) и метаданные сервиса
